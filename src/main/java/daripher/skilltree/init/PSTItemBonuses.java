...
public class PSTItemBonuses {
    ...
    public static final DeferredHolder<ItemBonus.Serializer, ItemBonus.Serializer> SKILL_BONUS = REGISTRY.register("skill_bonus", EquipmentBonus.Serializer::new);
    public static final DeferredHolder<ItemBonus.Serializer, ItemBonus.Serializer> ITEM_BONUS_LIST = REGISTRY.register("item_bonus_list", GroupedItemBonus.Serializer::new);


    @SuppressWarnings("rawtypes")
    public static List<ItemBonus> bonusList() {
        return PSTRegistries.ITEM_BONUSES.stream().map(ItemBonus.Serializer::createDefaultInstance)
                .map(ItemBonus.class::cast).toList();
    }

    public static String getName(ItemBonus<?> itemBonus) {
        ResourceLocation id = PSTRegistries.ITEM_BONUSES.getKey(itemBonus.getSerializer());
